...
import { PostBoardResponsetDto, GetBoardResponseDto, GetFavoriteListResponseDto, GetCommentListResponseDto, PostCommentResponseDto } from "./response/board";
...
export const signInReguest = async (requestBody: SignInRequestDto) => {
...
const POST_BOARD_UL = () => `${API_DOMAIN}/board`;
...
export const postBoardRequest = async (requestBody : PostBoardRequestDto, accessToken : string) => {
	const result = await axios.post(POST_BOARD_UL(), requestBody, authorization(accessToken))
	.then(response => {
		const responseBody : PostBoardResponsetDto = response.data;
		return responseBody;
	})
	.catch(error => {
		if (!error.response.data) return null;
