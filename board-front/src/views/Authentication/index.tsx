...
import { signInReguest, signUpReguest } from 'apis'
...
export default function Authentication() {
	...
	const SignInCard = () => {
		...
		const onSignInButtonClickHandler = () => {
			...
			signInReguest(requestBody).then(signInResponse);
		} 
	...
	const SignUpCard = () => {
		...
		const [passwordCheckButtonIcon, setPasswordCheckButtonIcon] = useState<'eye-light-on-icon' | 'eye-light-off-icon'>('eye-light-off-icon');


		//			function: 다음 주소 검색 함수				//
		...
		const signUpResponse = (responseBody : ResponseDto | null) => {
			...
			if (code === 'DT') {
				...
				setTelNumberErrorMessage('중복되는 휴대전화 번호입니다');
				setPage(2);
			}
		...
		//			event handler: 휴대전화 번호 변경 이벤트 처리				//
		...
		const onNextButtonClickHandler = () => {
			...
			if(!isEmailPattern || !isCheckedPassword || !isEqualsPassword) return;
			setPage(2);
		}
		...
		const onSignUpButtonClickHandler = () => {
			...
			if(!isTelNumberPattern) {
				setTelNumberError(true);
				setTelNumberErrorMessage('휴대전화 번호를 확인해주세요');
			}
			...
			signUpReguest(requestBody).then(signUpResponse);
		...
		//			event handler: 휴대전화 번호 키 다운 이벤트 처리				//
			...
			<div className='auth-card'>
				<div className='auth-card-box'>
					<div className='auth-card-top'>
						...
						{page === 1 && (
							<>
								<InputBox ref = {emailRef} label = '이메일주소*' type = 'text' placeholder='이메일 주소를 입력해주세요.' value={email} onChange={onEmailChangeHandler} error={isEmailError} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandler}/>
								<InputBox ref = {passwordRef} label = '패스워드*' type = {passwordType} placeholder='비밀번호를 입력해주세요.' value={password} onChange={onPasswordChangeHandler} error={isPasswordError} message={passwordErrorMessage} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler}/>
								<InputBox ref = {passwordCheckRef} label = '패스워드 확인*' type = {passwordCheckType} placeholder='비밀번호를 다시 입력해주세요.' value={passwordCheck} onChange={onPasswordCheckChangeHandler} error={isPasswordCheckError} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckKeyDownHandler}/>	
							</>
						)}
						{page === 2 && (
							<>
							<InputBox ref={nicknameRef} label='닉네임*' type='text' placeholder='닉네임을 입력 해주세요' value={nickname} onChange={onNicknameChangeHandler} error={isNicknameError} message={nicknameErrorMessage} onKeyDown={onNicknameKeyDownHandler}></InputBox>
							<InputBox ref={telNumberRef} label='휴대전화 번호*' type='text' placeholder='휴대전화 번호를 입력 해주세요' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}></InputBox>
							<InputBox ref={addressRef} label='주소*' type='text' placeholder='주소를 입력 해주세요' value={address} onChange={onAddressChangeHandler} error={isAddressError} message={addressErrorMessage} icon='expand-right-light-icon' onButtonClick={onAddressButtonClickHandler} onKeyDown={onAddressKeyDownHandler}></InputBox>
							<InputBox ref={addressDetailRef} label='상세주소' type='text' placeholder='상세주소를 입력 해주세요' value={addressDetail} onChange={onAddressDetailChangeHandler} error={false} onKeyDown={onAddressDetailKeyDownHandler}></InputBox>
							</>
						)}
					</div>
					<div className='auth-card-bottom'>
						...
						<div className='auth-description-box'>
							<div className='auth-description'>{'이미 회원이신가요?'}<span className='auth-description-link' onClick={onSignInLinkClickHandler}>{'로그인'}</span></div>
						</div>
					</div>
				</div>
			</div>
			...
			<div className='auth-jumbotron-box'>
				<div className='auth-jumbotron-contents'>
					...
					<div className='auth-jumbotron-text-box'>
						...
						<div className='auth-jumbotron-text'>{'SUMJO BOARD'}</div>
					</div>
				</div>
			</div>
